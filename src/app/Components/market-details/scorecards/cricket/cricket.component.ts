...
@Component({
  ...
  standalone: true,
  imports: [NgIf, NgFor, NgClass, ScoreCardSvgsComponent]
})
export class CricketComponent implements OnDestroy {
  @Input() sportObj: any = [];
  @Input() isMuted: any = [];
  ...
  getBetfairDataFirebase() {
    ...
    //   return
    // }
    this.ScoreCardSubscription = this.scoreFirebase.collection('cricketScore', ref => ref.where('eventId', '==', this.sportObj?.event_id))
      .stateChanges()
      .subscribe(changes => {
